...
{Browse {PartitionToTimedlist [[a b c] silence Drone(ChordOrNote:a Amount :10)]}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fun {Mix P2T Music}
   local
	 ...
	 [] H|T then
	    case H
	    of K|L then
	       if {Float.is K} then {MixHelper2 P2T T {Append H Acc}}
	       else {MixHelper2 P2T T {Append {MixBis {P2T H} 1.0 nil} Acc}}
	       end
	    ...
	    [] cut(start:D1 finish:D2 Music) then {Cut D1 D2 Music}
	       ...
	       if {Float.is H} then {MixHelper2 P2T T H|Acc}
	       else
		  {MixHelper2 P2T T {Append {MixBis {P2T [H]} 1.0 nil} Acc}}
	       end
	    end   
	 end
      end	 	 
      fun {MixHelper Note Index Acc}
	 local
	    TrueDuration = Note.duration*44100.0
	    Numvalue
	 in
	    ...
	    if Index == TrueDuration then Acc

	    else {MixHelper Note 1.0 (1.0/2.0)*{Float.sin (2.0*3.14159265359*{Number.pow 2.0 {Int.toFloat (Numvalue - 10) + (( Note.octave - 4)*12)}/12.0}*440.0*Index/44100.0)}|Acc}
	 ...
	 [] H|T then
	    case H
	    of K|L then {MixBis T (I + 1.0) {Append {FoldR {MixBis L I {Append {MixHelper K I nil} nil}} fun {$ X Y} X + Y end 0.0} A}}
	    else
		 {MixBis T (I + 1.0) {Append {MixHelper H I nil} A}}
	    end
	 end
      end
      fun{Reverse Music}
...
in
   local X = [a b c] 
      P2T = fun {$ Y} {PartitionToTimedList Y} end
   in
      {Browse {Mix P2T X}}
   end
end
